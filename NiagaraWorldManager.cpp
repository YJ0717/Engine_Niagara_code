...
void FNiagaraWorldManager::Tick(ENiagaraWorldManagerTickPass TickPass, ETickingGroup TickGroup, float DeltaSeconds, const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	const int32 ActualTickGroup = TickGroup - NiagaraFirstTickGroup;
	ActiveNiagaraTickGroup = ActualTickGroup;

	// Update the parameter collections.
	TickParameterCollections();
	
	// Warning: This is a temporary solution for FPS. 
	// For a production environment, you should use a more stable, averaged FPS value.
	const float FPS = 1.0f / FMath::Max(DeltaSeconds, KINDA_SMALL_NUMBER);

	// 카메라 거리와 프러스텀을 이용한 동적 시뮬레이션 티어 조절 시작
	if (CachedViewInfo.Num() > 0)
	{
		const FNiagaraCachedViewInfo& ViewInfo = CachedViewInfo[0];

		for (TObjectIterator<UNiagaraComponent> It; It; ++It)
		{
			UNiagaraComponent* Comp = *It;
			if (!Comp || !Comp->IsActive() || !Comp->GetAsset())
			{
				continue;
			}
			
			UWorld* LocalWorld = Comp->GetWorld();
			if (!LocalWorld || LocalWorld != GetWorld())
			{
				continue;
			}
			//정보 수집 단계
			const FVector ComponentLocation = Comp->GetComponentLocation();
			float MinDistanceSq = TNumericLimits<float>::Max();
			//모든 뷰를 순회하며 가장 가까운 카메라와의 거리를 계산
			for (const FNiagaraCachedViewInfo& LocalViewInfo : CachedViewInfo)
			{
				const float DistanceSq = FVector::DistSquared(LocalViewInfo.ViewToWorld.GetOrigin(), ComponentLocation);
				MinDistanceSq = FMath::Min(MinDistanceSq, DistanceSq);
			}
			const float DistanceToCamera = FMath::Sqrt(MinDistanceSq);
			// 컴포넌트의 경계구가 프러스텀 내에 조금이라도 걸치는지 확인.
			bool bIsInFrustum = false;
			const FBoxSphereBounds& Bounds = Comp->CalcBounds(Comp->GetComponentToWorld());
			for (const FPlane& FrustumPlane : ViewInfo.FrutumPlanes)
			{
				if (FrustumPlane.PlaneDot(Bounds.Origin) <= Bounds.SphereRadius)
				{
					bIsInFrustum = true;
					break;
				}
			...
			// 위에서 계산된 거리와 프러스텀 정보를 바탕으로 최종 시뮬레이션 상태를 결정.
			int32 SimulationTier = 0; // 0: Full, 1: Limited, 2: Kill
			float AlphaScale = 1.0f;

			if (DistanceToCamera > KeepAliveRadius)
			{
				//Kill Zone : 최대 반경을 벗어났으므로 제거 대상으로 설정.
				SimulationTier = 2; // Kill Zone
				AlphaScale = 0.0f;
			}
			else
			{
				if (!bIsInFrustum)
				{
					SimulationTier = 1; // Limited Simulation Zone , 화면 밖이지만, 근처에 있으므로 최소 연산 상태로 설정.
				}

				if (DistanceToCamera > FadeStartRadius) // Fade Out Zone 페이드 시작 거리를 지났다면 거리에 비례하여 알파 값을 1에서 0으로 줄임.
				{
					AlphaScale = 1.0f - FMath::Clamp((DistanceToCamera - FadeStartRadius) / (KeepAliveRadius - FadeStartRadius), 0.0f, 1.0f);
				}
			}
			
			//데이터 전달
			Comp->SetVariableInt(TEXT("User.SimulationTier"), SimulationTier);
			Comp->SetVariableFloat(TEXT("User.AlphaScale"), AlphaScale);
		}
	}


	// Run simulations that need to be ticked in this group.
...
// FName 캐싱
static const FName VarLODLevel(TEXT("User.LODLevel"));
static const FName VarLODScale(TEXT("User.LODScale"));
static const FName VarCPURate(TEXT("User.CPUEmitterSpawnRate"));
static const FName VarGPURate(TEXT("User.GPUEmitterSpawnRate"));
...
struct FPIDController
{
	...
	float Compute(float Target, float Current, float DeltaTime)
	{
		...
		Integral = FMath::Clamp(Integral + (Error * DeltaTime), -50.0f, 50.0f);
		float Derivative = (DeltaTime > SMALL_NUMBER) ? (Error - PreviousError) / DeltaTime : 0.f;
		PreviousError = Error;
...
static FPIDController GlobalPID;

void FNiagaraWorldManager::ExecuteSimulations(
	...
	const FGraphEventRef& MyCompletionGraphEvent)
{
	...
	// [PID 기반 FPS 보정]
	const float CurrentFPS = (DeltaSeconds > 0.001f) ? 1.0f / DeltaSeconds : 60.0f;
	const float TargetFPS = 60.0f;
	float PIDVal = GlobalPID.Compute(TargetFPS, CurrentFPS, DeltaSeconds);
	//시그모이드 넣기
	float GlobalFPSFactor = 1.0f - Sigmoid(PIDVal * 0.1f) * 1.5f + 0.5f;
	GlobalFPSFactor = FMath::Clamp(GlobalFPSFactor, 0.1f, 1.0f);
		...
		for (FConstPlayerControllerIterator It = MyWorld->GetPlayerControllerIterator(); It; ++It)
		{
			if (APlayerController* PC = It->Get())
			{
				if (PC->PlayerCameraManager)
				{
					...
					CamDir = PC->PlayerCameraManager->GetActorForwardVector();
					bHasCamera = true;
					break;
				}
			}
	...
	{
		// 카메라 후방컬링
		static const float BackCullThreshold = -0.1f;

		for (TObjectIterator<UNiagaraComponent> It; It; ++It)
		{
			UNiagaraComponent* Comp = *It;
			if (!Comp || !Comp->IsRegistered() || !Comp->IsActive() || Comp->GetWorld() != MyWorld)
				continue;

			UNiagaraSystem* NiagaraAsset = Comp->GetAsset();
			if (!NiagaraAsset)
				continue;

			FVector CompLoc = Comp->GetComponentLocation();
			float Distance = FVector::Dist(CamLoc, CompLoc);
			int32 LODLevel = 0;

			//후방 컬링

			FVector DirToComp = (CompLoc - CamLoc).GetSafeNormal();
			float DotProduct = FVector::DotProduct(CamDir, DirToComp);

			if (DotProduct < BackCullThreshold)
			{
				LODLevel = 5;
			}
			else
			{
				if (Distance > 5000.f || CurrentFPS < 25.f) LODLevel = 5;
				else if (Distance > 4000.f || CurrentFPS < 30.f) LODLevel = 4;
				else if (Distance > 3000.f || CurrentFPS < 40.f) LODLevel = 3;
				else if (Distance > 2000.f || CurrentFPS < 50.f) LODLevel = 2;
				else if (Distance > 1000.f || CurrentFPS < 60.f) LODLevel = 1;
				else LODLevel = 0;
			}


			static const float CPURateTable[] = { 1.0f, 0.8f, 0.6f, 0.4f, 0.2f, 0.0f };
			static const float GPURateTable[] = { 0.0f, 0.2f, 0.4f, 0.6f, 0.8f, 1.0f };

			float TargetCPURate = CPURateTable[LODLevel] * GlobalFPSFactor;
			float TargetGPURate = GPURateTable[LODLevel] * GlobalFPSFactor;

			for (const FNiagaraEmitterHandle& EmitterHandle : NiagaraAsset->GetEmitterHandles())
			{
				const FVersionedNiagaraEmitter& VersionedEmitter = EmitterHandle.GetInstance();
				UNiagaraEmitter* NiagaraEmitter = VersionedEmitter.Emitter;
				if (!NiagaraEmitter) continue;

				const FVersionedNiagaraEmitterData* EmitterData = NiagaraEmitter->GetLatestEmitterData();
				if (!EmitterData || !EmitterData->bHLODTogle)
					continue;

				// 파라미터 적용
				Comp->SetVariableInt(VarLODLevel, LODLevel);
				Comp->SetVariableFloat(VarLODScale, TargetCPURate);
				Comp->SetVariableFloat(VarCPURate, TargetCPURate);
				Comp->SetVariableFloat(VarGPURate, TargetGPURate);

				// 디버그 로그
				UE_LOG(LogNiagara, Log,
					TEXT("[LOD] %s | %s | LOD=%d | Dist=%.0f | Dot=%.2f | FPS=%.1f | CPU=%.2f | GPU=%.2f"),
					*Comp->GetName(),
					*EmitterHandle.GetName().ToString(),
					LODLevel, Distance, DotProduct, CurrentFPS, TargetCPURate, TargetGPURate);
			}
		}
	}
